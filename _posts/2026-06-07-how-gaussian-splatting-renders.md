...
- Color: usually spherical harmonics rather than flat RGB, so color shifts with viewing angle. Evaluating it is a weighted sum of basis functions. Cheap, and not a neural network.
...
That duplication sounds wasteful, but it buys independence: every tile ends up with a self-contained list of the splats that affect it. Tiles never talk to each other, so the GPU can process thousands of them in parallel with no coordination.
...
None of these is new on its own. EWA splatting, tile-based rasterization, radix sort, alpha compositing all predate the 2023 paper. What Gaussian Splatting did was assemble them around an explicit, differentiable representation and tune the whole thing until it trained in minutes and rendered in real time. The breakthrough is the combination.
